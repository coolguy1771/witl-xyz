...
class App {
  ...
  constructor(
    ...
    console.log("Debug: App constructor completed", {
      mediasLength: this.medias.length,
      rendererExists: !!this.renderer,
      canvasExists: !!this.renderer?.gl?.canvas
    });
  }
  ...
  createRenderer() {
    try {
      ...
      console.log("Debug: Renderer created successfully", {
        canvasWidth: this.renderer.gl.canvas.width,
        canvasHeight: this.renderer.gl.canvas.height
      });
      this.container.appendChild(this.renderer.gl.canvas as HTMLCanvasElement);
      console.log("Debug: Canvas appended to container");
    } catch (error) {
      console.error("Debug: Error creating renderer:", error);
      throw error;
    }
  }
  ...
  createMedias(
    ...
    console.log("Debug: createMedias called", {
      items: items?.slice(0, 2),
      itemsLength: items?.length,
      bend,
      textColor,
    ...
    console.log("Debug: Gallery items selected", {
      useDefaultItems: !items || !items.length,
      galleryItemsLength: galleryItems.length,
      firstTwoItems: galleryItems.slice(0, 2)
    });
    
    this.mediasImages = galleryItems.concat(galleryItems);
    console.log("Debug: Medias images created", {
      mediasImagesLength: this.mediasImages.length
    });
    ...
    console.log("Debug: Media objects created", {
      mediasLength: this.medias.length
    });
  }
  ...
  onWheel(e: Event) {
    ...
    const delta =
      wheelEvent.deltaY ||
      ("wheelDelta" in wheelEvent ? (wheelEvent as WheelEvent & { wheelDelta: number }).wheelDelta : 0) ||
      ("detail" in wheelEvent ? (wheelEvent as WheelEvent & { detail: number }).detail : 0);
...
export default function CircularGallery({
  ...
  console.log("Debug: CircularGallery component rendered", {
  ...
  useEffect(() => {
    console.log("Debug: CircularGallery useEffect", {
      containerRefCurrent: !!containerRef.current,
      itemsLength: items?.length,
    ...
    if (!containerRef.current) {
      console.log("Debug: No container ref, returning early");
      return;
    }
    
    console.log("Debug: Creating new App instance");
    ...
    return () => {
      console.log("Debug: Destroying App instance");
      app.destroy();
    };
